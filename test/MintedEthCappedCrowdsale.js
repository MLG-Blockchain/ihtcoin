...
contract('MintedEthCappedCrowdsale: Success Scenario', function(accounts) {
    ...
    var _crowdsaleAllocateAgent = accounts[1];
    ...
    beforeEach(async() => {
        ...
        tokenVestingInstance = await TokenVesting.new(tokenInstance.address);
        pricingInstance = await Pricing.new(_tranches);
        multisigWalletInstance = await MultisigWallet.new(_listOfOwners, _minRequired);
        crowdsaleInstance = await Crowdsale.new(tokenInstance.address, pricingInstance.address, multisigWalletInstance.address, _startTime, _endTime, _minimumFundingGoal, _cap, tokenVestingInstance.address);
    ...
    it('Preallocate: Calling preallocate by non-owner should fail.', async function() {
        await tokenInstance.setMintAgent(crowdsaleInstance.address, true);
        await tokenInstance.setMintAgent(finalizeAgentInstance.address, true);
        await tokenInstance.setReleaseAgent(finalizeAgentInstance.address);
        await tokenInstance.setTransferAgent(crowdsaleInstance.address, true);
        await tokenInstance.setTransferAgent(tokenVestingInstance.address,true);
        await tokenVestingInstance.setAllocateAgent(crowdsaleInstance.address, true);
        await crowdsaleInstance.setFinalizeAgent(finalizeAgentInstance.address);
        await crowdsaleInstance.setAllocateAgent(accounts[2],true);
        ...
        var tokensPurchased = tokenInSmallestUnit(50000, _tokenDecimals);
        var principleLockAmout = tokenInSmallestUnit(10000, _tokenDecimals);
        var bonusLockAmount =  tokenInSmallestUnit(5000, _tokenDecimals);
        var principlelockPeriod = 120;
        var bonusLockPeriod = 180;
        var tokenPriceInWei = tokenPriceInWeiFromTokensPerEther(2000);
        try{
            await crowdsaleInstance.preallocate(buyer, tokensPurchased, tokenPriceInWei, principleLockAmout, principlelockPeriod, bonusLockAmount, bonusLockPeriod, { from: accounts[1] });
        } catch (error) {
                return assertJump(error);
            }
            assert.fail('should have thrown exception before');
    ...
    it('Preallocate: Preallocate the tokens with owner must fail', async function() {
        await tokenInstance.setMintAgent(crowdsaleInstance.address, true);
        await tokenInstance.setMintAgent(finalizeAgentInstance.address, true);
        await tokenInstance.setReleaseAgent(finalizeAgentInstance.address);
        await tokenInstance.setTransferAgent(crowdsaleInstance.address, true);
        ...
        await tokenVestingInstance.setAllocateAgent(crowdsaleInstance.address, true);
        await crowdsaleInstance.setFinalizeAgent(finalizeAgentInstance.address);
        await crowdsaleInstance.setAllocateAgent(accounts[3],true);
        assert.equal(await crowdsaleInstance.isFinalizerSane(), true, "Finalizer not sane. Can't continue.");
        assert.equal(await crowdsaleInstance.isPricingSane(), true, "Pricing not sane. Can't continue.");
        ...
        var tokenPriceInWei = tokenPriceInWeiFromTokensPerEther(2000);
        try{
            await crowdsaleInstance.preallocate(buyer, tokensPurchased, tokenPriceInWei, principleLockAmout, principlelockPeriod, bonusLockAmount, bonusLockPeriod, { from: accounts[0] });
        } catch (error) {
                return assertJump(error);
            }
            assert.fail('should have thrown exception before');
    ...
    it('Preallocate: allocating the tokens with allocate agent must increase the balance in crowdsale contract', async function() {
        await tokenInstance.setMintAgent(crowdsaleInstance.address, true);
        await tokenInstance.setMintAgent(finalizeAgentInstance.address, true);
        await tokenInstance.setReleaseAgent(finalizeAgentInstance.address);
        await tokenInstance.setTransferAgent(crowdsaleInstance.address, true);
        ...
        await crowdsaleInstance.preallocate(buyer, tokensPurchased, tokenPriceInWei, principleLockAmout, principlelockPeriod, bonusLockAmount, bonusLockPeriod, { from: accounts[2] });
        assert.equal(web3.toBigNumber(await tokenInstance.balanceOf(buyer)).toNumber(), tokenInSmallestUnit(35000, _tokenDecimals), "Assert 1 Failed");
        assert.equal(web3.toBigNumber(await crowdsaleInstance.tokensSold.call()).toNumber(), tokensPurchased, "Assert 2 Failed");
    });

    it('Preallocate: allocating the tokens with allocate agent must increase the balance in vesting contract', async function() {
        await tokenInstance.setMintAgent(crowdsaleInstance.address, true);
        await tokenInstance.setMintAgent(finalizeAgentInstance.address, true);
        await tokenInstance.setReleaseAgent(finalizeAgentInstance.address);
        await tokenInstance.setTransferAgent(crowdsaleInstance.address, true);
        ...
        await crowdsaleInstance.preallocate(buyer, tokensPurchased, tokenPriceInWei, principleLockAmout, principlelockPeriod, bonusLockAmount, bonusLockPeriod, { from: accounts[2] });
        assert.equal(web3.toBigNumber(await tokenInstance.balanceOf(tokenVestingInstance.address)).toNumber(), tokenInSmallestUnit(15000, _tokenDecimals), "Assert 1 Failed");
    });
    ...
    it('Whitelist: Allocate agent must be able to update early participants whitelist', async function() {
        await crowdsaleInstance.setAllocateAgent(accounts[1],true);
        await crowdsaleInstance.setEarlyParicipantWhitelist(accounts[4], true, {from : accounts[1]});
        assert.equal(await crowdsaleInstance.earlyParticipantWhitelist.call(accounts[4]), true);
    });

    it('Whitelist: Non-owner should not be able to set participants list', async function() {
        try {
    ...
    it('Whitelist: Only whitelisted address should be able to contribute even before the funding start.', async function() {
        ...
        // await crowdsaleInstance.buy({ from: accounts[3], value: web3.toWei('2', 'ether') });
        // var balance = await tokenInstance.balanceOf(accounts[3]);
        // assert.equal(balance, tokenInSmallestUnit(1304, _tokenDecimals),"Balance is"+balance);
