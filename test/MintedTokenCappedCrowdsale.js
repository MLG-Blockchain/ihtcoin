...
contract('MintedTokenCappedCrowdsale: Success Scenario', function(accounts) {
    ...
    var _cap = tokenInSmallestUnit(100000,_tokenDecimals);
    var _teamBonusPoints = [200, 200, 200];
    var _teamAddresses = [accounts[0], accounts[1], accounts[2]];
    var decimals = _tokenDecimals;
    var _tranches = [
        tokenInSmallestUnit(0), tokenPriceInWeiFromTokensPerEther(1500),
        ...
        tokenInSmallestUnit(75000,_tokenDecimals), tokenPriceInWeiFromTokensPerEther(1000),
        _cap, 0
    ];
    ...
    //     var tokensPurchased = etherInWei(50000, _tokenDecimals);
    ...
    //     await crowdsaleInstance.preallocate(buyer, tokensPurchased, tokenPriceInWei, { from: accounts[2] });
    //     assert.equal(web3.toBigNumber(await tokenInstance.balanceOf(buyer)).toNumber(), tokensPurchased, "Assert 1 Failed");
    //     assert.equal(web3.toBigNumber(await crowdsaleInstance.tokensSold.call()).toNumber(), tokensPurchased, "Assert 2 Failed");
    //     assert.equal(web3.toBigNumber(await crowdsaleInstance.weiRaised.call()).toNumber(), etherInWei(25), "Assert 3 Failed");
    // });
